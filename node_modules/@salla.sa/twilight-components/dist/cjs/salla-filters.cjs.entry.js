...
const SallaFilters = class {
    ...
    render() {
        return index.h(index.Host, { key: '3b38160573591da8249606b8c12b72b58d03551d' }, this.filters?.map(option => index.h("salla-filters-widget", { option: option, filtersData: this.filtersData, onChanged: ({ detail: { event, option, value } }) => {
                this.handleOptionChange(event, option, value);
            } })), this.filters?.length ? index.h("div", { class: "s-filters-footer" }, index.h("salla-button", { color: 'gray', fill: 'outline', onClick: () => this.resetFilters() }, this.reset)) : '');
    }
    componentDidLoad() {
        this.isReady = true;
    }
