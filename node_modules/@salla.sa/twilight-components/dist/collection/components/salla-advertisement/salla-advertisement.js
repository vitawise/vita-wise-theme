...
export class SallaAdvertisement {
    ...
    */
    constructor() {
        ...
        this.advSlot = this.host.querySelector('[slot="adv"]')?.innerHTML || `<div class="s-advertisement-content"><span class="s-advertisement-content-main">{iconElem}{urlElem}</span>{closeElem}</div>
    `;
    }
    /**
    ...
    isNotVisible(advert) {
        ...
        // ? Show advertisement in preview mode
        if (isPreview)
            return false;
    ...
    */
    componentWillLoad() {
        const getData = async () => {
            // * Note: Do not show advertisements or load data in mobile app
            const isMobileApp = salla.config.isMobileApp();
            if (isMobileApp)
                return [];
            const isPreview = salla.config.isDebug() || salla.helpers.isPreview();
            // Wait for salla to be ready
            await new Promise(resolve => salla.onReady(resolve));
            // Fetch advertisements based on the current page
            const pageSlug = !isPreview ? this.currentSlug : null;
            const response = await salla.api.advertisement.fetch(pageSlug);
            if (Array.isArray(response.data)) {
                const advertisements = response.data;
                // Show all active advertisements in preview mode
                if (isPreview) {
                    this.advertisements = advertisements;
                }
                else {
                    this.advertisements = advertisements.filter(ad => !salla.storage.get(`statusAd-${ad.id}`));
                }
            }
            else {
                this.advertisements = [];
    ...
    static get is() { return "salla-advertisement"; }
    static get originalStyleUrls() {
        return {
            "$": ["salla-advertisement.scss"]
        };
    }
    static get styleUrls() {
        return {
            "$": ["salla-advertisement.css"]
        };
    }
    static get states() {
        return {
